...
export function playerWon(winner, role) {
...
function scoreDelta({ role, won, previousBestStreak, nextCurrentStreak }) {
  const breakdown = [{ label: 'Jugar ronda', points: SCORE_POINTS.participation }]
  if (won) breakdown.push({ label: 'Victoria', points: SCORE_POINTS.win })
  if (won && isImpostorRole(role)) breakdown.push({ label: 'Victoria impostor', points: SCORE_POINTS.impostorWin })
  if (won && nextCurrentStreak > previousBestStreak) {
    breakdown.push({ label: 'Mejor racha', points: SCORE_POINTS.newBestStreak })
  }
...
export function buildScoreSummary(players = [], winner, previousScores = {}) {
  const nextScores = { ...(previousScores || {}) }
  const roundPoints = players.map(player => {
    const previous = nextScores[player.id] || emptyScore(player)
    const role = player.role || 'citizen'
    const won = playerWon(winner, role)
